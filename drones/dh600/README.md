...
flies: fit the A8 mini, CompassMot, and a 30 s PosHold hover at the final weight, which also
verifies the final gains (the post-tune logs show no oscillation but only two calm seconds). ArduPilot Copter 4.7.1, configuration captured in
...
not match the index order.

**Status column is load-bearing:** ✅ = physically wired and confirmed on the aircraft; ⬜ = planned
only, never verified. Do not configure against a ⬜ row without eyes on the board first.

| Port | UART | `SERIALx` | Device | Status | Notes |
|------|------|-----------|--------|--------|-------|
...
| CAN1 / CAN2 | — | — | spare | — | |
| I2C, 2× debug | — | — | spare | — | |
| Power 1 / Power 2 | — | — | PM07 on Power 1 | ✅ wired | second analog input available for redundant supply |

> ⚠ **`BRD_SER1_RTSCTS=0` is mandatory with ELRS on TELEM1.** TELEM1 is the one port with hardware
> flow control actually wired (`UART7_RTS`/`UART7_CTS`). CRSF does not use it, and the ArduPilot
> default of `2` (auto) can produce a dead or intermittent link. This did not apply under the old
> GPS2 plan, because UART8 has no flow-control pins.

> CRSF is preferred over S.Bus into RC IN because it is bidirectional — RSSI, battery and GPS come
...
- **2026-09-18** — **Harmonic notch measured and verified in two hovers.** No RPM source on this
  ...
  max saved on all three. Tuning complete. Logs checked afterwards: all three tunes converged
  normally (final twitches within ±5 % of target); yaw is authority-limited (rate target never
  reached, filter at its 1 Hz floor, accel max 183 vs 270 default), which is the frame, not the
  tune. Post-tune flights show no oscillation, but the only flight on the final gains was a 17 s
  stick-heavy hop, so the final-gain check rides on the final-weight hover.
